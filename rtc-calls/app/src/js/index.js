...
onopenMiro('https://miro.com/app/board/uXjVMQ_om3w=/?share_link_id=870054768292')
...
if (location.hash === '#init') {
    ...
    navigator.mediaDevices.getUserMedia({ video: true, audio: true }).then((stream) => {
        ...
        var ws = new WebSocket(`ws://${location.hostname}/api/ws/${location.search.split('=')[1]}`);
        ...
        peer.on('signal', function(data) {
            ws.send(JSON.stringify({ type: 'new_connection', data: JSON.stringify(data) }))
        })
        ...
        document.getElementById('novideobtn').addEventListener('click', function() {
            if (is_camera) {
                stream.getTracks().forEach(function(track) {
                    if (track.kind === "video")
                        track.enabled = false
                });
                document.getElementById('novideobtn').classList.add("footer-btn-off")
            } else {
                stream.getTracks().forEach(function(track) {
                    if (track.kind === "video")
                        track.enabled = true
                });
                document.getElementById('novideobtn').classList.remove("footer-btn-off")
            }
            is_camera = !is_camera
        })
    ...
    navigator.mediaDevices.getUserMedia({ video: true, audio: true }).then((stream) => {
        ...
        peer.on('signal', function(data) {
            ws.send(JSON.stringify({ type: 'connect', data: JSON.stringify(data) }))
        })
